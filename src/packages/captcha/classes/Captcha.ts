import { Canvas, createCanvas, PNGStream, JPEGStream } from "canvas";
import { randomCaptchaText, shuffleArray } from "@packages/captcha";
...
export class Captcha {
	...
	constructor(_h: number = 250) {
		...
		// Initialize canvas
		this._canvas = createCanvas(400, _h);
		...
		ctx.translate(
			...
			-1 * Math.round(Math.random() * (_h / 4) - _h / 6) - _h / 2
		);
		ctx.rotate(Math.random() - 0.5);
		// Set text value and print it to canvas
		ctx.beginPath();
		this._value = "";
		while (this._value.length !== 6) this._value = randomCaptchaText();

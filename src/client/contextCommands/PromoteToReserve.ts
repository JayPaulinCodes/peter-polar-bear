...
export default new ContextCommand({
    data: new ContextMenuCommandBuilder()
        .setName("Promote to Reserve Deputy")
    ...
    run: async ({ client, interaction }) => {
        ...
        const deputyRole = tryGetRoleByName(client, RoleName.RESERVE_DEPUTY);
        ...
        // Log to fto log channel
        await ftoLog(client, `${member.nickname ?? member.displayName} (${member.id}) has promoted ${target.nickname ?? target.displayName} (${target.id}) to reserve deputy`);
        
        // Follow Up
        await interaction.editReply("User successfully promoted to reserve deputy!")
    }
});

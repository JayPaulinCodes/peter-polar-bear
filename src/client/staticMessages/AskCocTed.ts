...
export default new StaticMessage({
    ...
    update: async (client: ExtendedClient) => {
        client.logger.debug("Starting update for static message 'askCocTed'");
        const GUILD_ID = homeGuildId();
        const CHANNEL_ID = tryGetChannelByName(client, ChannelName.TED_ASK_COC)?.id ?? "1176658796668522496";
        const DB_NAME = "askCocTed";

        const [ embed, attachment ] = client.embeds.subdivisionCoC(await client.guilds.fetch(GUILD_ID), "TED");
        ...
        client.logger.log("Finished update for static message 'askCocTed'");
    }
});
